...
n = np.cross(AB,AC).ravel();        

n = n/np.linalg.norm(n)         # Face normal
print(n)
...
plt.figure(100)
plt.imshow(tri_shading, 'gray')
plt.title("Polygon Shading Image")
...
# interpolate
F_p = interp2(fx_pb, fy_pb, F_pb, u, v, method='linear', fill_value=0)
...
plt.figure(2)
plt.imshow(np.log(np.abs(H0)), 'gray')
...
iH =  ASM_prop(H0, A[2,0], dp, lam, radius=0.1)

# plt.figure(3)
# plt.imshow(np.log(abs(iH)),'gray')
# plt.title("reconstruction image")
# plt.show()
# 修正案：対数(log)をとらずに、光の強度(絶対値の2乗)をそのまま表示する
plt.figure(3)
intensity_image = np.abs(iH)**2
# 上位数%の白飛びを抑えてコントラストを調整して表示
v_max = np.percentile(intensity_image, 99.5) 
plt.imshow(intensity_image, 'gray', vmax=v_max)
plt.title("reconstruction image (Intensity without log)")
plt.show()