...
pre = U**2 + V**2
G_total = np.zeros((Ny*2, Nx*2), dtype=np.complex64)
g_pad = np.zeros((Ny*2, Nx*2), dtype=np.complex128)

model_name = os.path.join("cube.ply")
...
# i=0
    ...
    A = Global_vertices[Global_faces[i, 0]];     B = Global_vertices[Global_faces[i, 1]];         C = Global_vertices[Global_faces[i, 2]];               # vertices coordinates: A, B, C 

    AB = (B-A).T;         AC = (C-A).T      #  vectors AB and AC
    n_vector = np.cross(AB,AC).ravel()
    n_vector = n_vector/np.linalg.norm(n_vector)         # Face normal
    ...
    if (n_vector != 0).any():
        ...
        # f_view = np.log(np.abs(f_pad))
        # plt.figure(i)
        # plt.imshow(f_view, 'gray')
        # plt.show()
        ...
        # g_subpad = np.zeros((Ny, Nx), dtype=np.complex128)
        # g_subpad = np.fft.fftshift(np.fft.ifft2(np.fft.ifftshift(G)))
        # X_start, Y_start = Nx // 2, Ny // 2
        # g_sub = g_subpad[Y_start : Y_start + Ny, X_start : X_start + Nx]
        # g_subabs = np.log(np.abs(g_sub)+1)
        # if g_subabs.max() > 0:
        #     g_subview = (255 * (g_subabs / g_subabs.max())).astype(np.uint8)
        # else:
        #     g_subview = np.zeros_like(g_subabs, dtype=np.uint8)

        # plt.figure(i)
        # plt.imshow(g_subview, 'gray')
        # plt.show()
        G_total += G
        # g_pad += np.fft.fftshift(np.fft.ifft2(np.fft.ifftshift(G)))
        

        # F_view = np.log(np.abs(G) + 1)
        # spectrum_view = (255 * (F_view - F_view.min()) / (F_view.max() - F_view.min())).astype(np.uint8)
        # Image.fromarray(spectrum_view, mode="L").save("triple_triangle_Global_spectrum.bmp", format="BMP")


# plt.imshow(np.abs(np.log(G_total+1)), 'gray')
# plt.show()

g_pad = np.fft.fftshift(np.fft.ifft2(np.fft.ifftshift(G_total)))


# print(np.max(g_pad), np.min(g_pad))
# print(np.max(np.abs(g_pad)), np.min(np.abs(g_pad)))

g_abs = np.abs(g_pad)

# plt.imshow(g_abs, 'gray')
# plt.show()

if g_abs.max() > 0:
    g_view = (255 * (g_abs / g_abs.max())).astype(np.uint8)
else:
    g_view = np.zeros_like(g_abs, dtype=np.uint8)

Image.fromarray(g_view, mode="L").save("cube_pad.bmp", format="BMP")

# 2048×2048を1024×1024に切り出し
g = np.zeros((Ny, Nx), dtype=np.complex128)
X_start, Y_start = Nx // 2, Ny // 2
g = g_pad[Y_start : Y_start + Ny, X_start : X_start + Nx]

g_abs = np.log(np.abs(g)+1)
if g_abs.max() > 0:
    g_view = (255 * (g_abs / g_abs.max())).astype(np.uint8)
else:
    g_view = np.zeros_like(g_abs, dtype=np.uint8)
...
plt.show()
Image.fromarray(g_view, mode="L").save("cube.bmp", format="BMP")
