...
# model_name = "triple_triangle3.ply" # ファイル名を指定
model_name = "single_triangle10_rot.ply" # ファイル名を指定
# model_name = "texbunny_lowpoly_300.ply" # ファイル名を指定
# model_name = "stuck_triangle.ply" # ファイル名を指定

model = os.path.join(debug_path, "debug_ply", model_name)
data = trimesh.load(model)

R_rot = Rotation_matrix(np.radians(60))
...
# save_path = os.path.join(debug_path, "debug_bmp", "triple_triangle3_hologram.bmp")
save_path_bmp = os.path.join(debug_path, "debug_bmp", "single_triangle10_rot_hologram.bmp")
save_path_npy = os.path.join(debug_path, "debug_npy", "single_triangle10_rot_hologram.npy")
# save_path = os.path.join(debug_path, "debug_bmp", "single_triangle6_hologram.bmp")
Image.fromarray(g_view, mode="L").save(save_path_bmp, format="BMP")
np.save(save_path_npy, g)
...
# G_recon = G * Hz(z-z*np.sin(np.radians(60)))
...
# plt.imshow(g_view, 'gray')
plt.figure(4)
plt.imshow(np.log(np.abs((np.fft.fftshift(np.fft.ifft2(np.fft.ifftshift(G_total*Hz(2+np.sin(np.radians(60)))))))[Y_start : Y_start+Ny, X_start : X_start+Nx])+1), 'gray')
plt.figure(5)
plt.imshow(np.log(np.abs((np.fft.fftshift(np.fft.ifft2(np.fft.ifftshift(G_total*Hz(2-np.sin(np.radians(60)))))))[Y_start : Y_start+Ny, X_start : X_start+Nx])+1), 'gray')
plt.show()