...
for method in method_name:
    holo_original = os.path.join(research_path, "debug_experiment", method, "single_triangle", "original")
    holo_interpolation = os.path.join(research_path, "debug_experiment", method, "single_triangle", "interpolation")

    model_interp_npy = os.path.join(holo_interpolation, "npy") # C:\Users\YutoMatsuo\Desktop\Research-debug\debug_experiment\cubic\single_triangle\interpolation\npy
    model_interp_bmp = os.path.join(holo_interpolation, "bmp") # C:\Users\YutoMatsuo\Desktop\Research-debug\debug_experiment\cubic\single_triangle\interpolation\bmp
    model_ori = os.path.join(holo_original, "npy", "045_+00.npy")
    g = np.load(model_ori)
    ...
    for n, deg in enumerate(range(-45, 46)):
        ...
        holo_save_npy = os.path.join(model_interp_npy, f"{n:03d}_{deg:+03d}.npy")
        holo_save_bmp = os.path.join(model_interp_bmp, f"{n:03d}_{deg:+03d}.bmp")

        np.save(holo_save_npy, g_recon_rotate)
        Image.fromarray(h.normalize_255("log", g_recon_rotate), mode="L").save(holo_save_bmp, format="BMP")
        ...
        # plt.figure(4)
        # plt.imshow(h.normalize_255("log", g_recon_rotate_right), 'gray')
        # plt.show()
