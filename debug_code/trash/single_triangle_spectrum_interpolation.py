...
#ゼロパディング後の画像でのピクセルでの重心の位置計算
Nx_hat_gp = ((Local_gravitypoints[0] - x_hat_min)/pitch + x_start)
Ny_hat_gp = ((Local_gravitypoints[1] - y_hat_min)/pitch + y_start)

# 位相シフト計算
phase_shift = np.exp(1j * 2 * np.pi * ((Nx_hat_origin - Nx_hat_gp)*U_hat + (Ny_hat_origin - Ny_hat_gp)*V_hat))
...
y_interp_w = R[1, 0]*U + R[1, 1]*V + R[1, 2]*W - R[1, 2]/WAVELENGTH

query_points = np.stack((x_interp_u.ravel(), y_interp_w.ravel()), axis=-1)

F_interp = interp(query_points).reshape(Ny*2, Nx*2)

F_view = np.log(np.abs(F_interp) + 1)
spectrum_view = (255 * (F_view - F_view.min()) / (F_view.max() - F_view.min())).astype(np.uint8)
Image.fromarray(spectrum_view, mode="L").save("single_triangle_spectrum_interpolation.bmp", format="BMP")